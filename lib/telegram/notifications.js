...
function sendToChats(event, chats, callback) {
	...
	async.eachSeries(chats, (chat, callback) => {
		...
		if (textOnly) {
			...
			// Send a text message
			logger.info(`Sending text notification to <${chat.id}> for event <${event.id}>, min_distance <${chat.min_distance}>`);
			...
			logger.info(`Sending image notification to <${chat.id}> for event <${event.id}>, min_distance <${chat.min_distance}>`);
