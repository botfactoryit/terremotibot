		...
		geocoding.reverse(lat, lon, (err, result) => {
			...
			card.generate((err, filePath) => {
				...
				if (magnitude >= BROADCAST_THRESHOLD &&
					// broadcast only if the earthquake is in a radius from the "center of Italy"
					_distanceFromCenter(lat, lon) <= BROADCAST_RADIUS) {
...
function __distanceFromCenter(lat, lon) {
